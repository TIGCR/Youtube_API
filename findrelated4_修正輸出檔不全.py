...
for program in program_list:
    data = cut.read_videoID(program,'2021-11-01','2021-11-11',words_antiCH) #讀取該頻道下所有符合篩選字的video
    #dataFilter = cut.select_date(df, '2019-03-29', '2020-01-11') 篩選日期
    allissueword = cut.read_keyword_data("issue","senti") #載入需要的詞庫
    dataSegment = cut.jieba_cutwords(data,"issueword","sentiword",language=True) #載入需要的詞庫.txt
...
cut.tfidf(['少康戰情室'],'2021-11-01','2021-11-10') #計算少康戰情室2020年的tf-idf分數
...
result = eval('double_table'+str(i))
    d = {}
    for b,v in result.values:
        d[b] =v
    wave_mask = np.array(Image.open("princess.jpg"))
        # 產生文字雲
    wordcloud = WordCloud(font_path="NotoSansCJKtc-Black.otf", #設置字體
